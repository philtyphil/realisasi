...
class Wilayah extends CI_Controller {
	...
	public function index()
	{
		...
		$auth_page = $this->menuroleaccess->check_access("master");
		if($auth_page) 
		{
			$this->load->model('wilayah_model');
			/** Success Login **/
			$data['sForm'] 				= "Master Wilayah";
			$data['title'] 				= "Master Wilayah";
			$data['master']				= $this->wilayah_model->get_master();
			$data['golongan']			= array();
			$data['status_pegawai']		= array();
			$data['pendidikan']			= array();
			$data['status_keluarga'] 	= array();
			
			render('wilayah',$data,"wilayah");
		}
		else
		{
	...
	public function edit()
	{
		...
		$buff	 = array();
		$this->load->model('wilayah_model');
		$data 	 = $this->wilayah_model->edit($id);
		if($data)
		{
			$data 					= $data[0];
			$buffer['code_wilayah'] = $data['ID_CONFIG'];
			$buffer['title']		= "Master Edit";
			$buffer['nm_wilayah']	= $data['NM_CONFIG'];
			$buffer['desc']			= $data['KET'];
		}
		
		render('wilayah_edit',$buffer,'wilayah_edit');
	}
	...
	{
		$this->load->library('menuroleaccess');
		$auth_page = $this->menuroleaccess->check_access("wilayah");
		if($auth_page && $this->input->post())
		{
			$this->load->library('form_validation');
			$this->form_validation->set_rules('group', 'Group', 'trim|required|htmlentities');
			...
			if($this->form_validation->run() == FALSE)
			{
				if(validation_errors('group')!=NULL){
					$view['error_group'] 		= strip_tags(form_error('group'));
				}
				if(validation_errors('wilayah')!=NULL){
					$view['error_wilayah'] 		= strip_tags(form_error('wilayah'));
				}
			...
			else
			{
				...
				$data	= $this->wilayah_model->insert($insert);
	 ...
	 *  @philtyphils
	 *  
	 *  @return json data, datatables to view wilayah table
	 *  
	...
	public function json()
	{
		...
		$this->load->model('wilayah_model');
		$data = $this->wilayah_model->json_dt($_GET,$id_master,$level);
	...
	public function rekap()
	{
		...
		if($auth_page && $this->input->post())
		{
			...
			$this->load->library('form_validation');
			$this->form_validation->set_rules('id_master', 'Code/Nama', 'trim|numeric');
			...
			else
			{
				...
				$buff['template']				= template();
				$view['table_rekap_pegawai']	= $this->parser->parse(template().'/jLoadpage/wilayah_content.html',$buff);
				$view['success']				= "UYEAY";
